...
func init() {
	fmt.Println("여기는 init 함수 시작부 입니다")
	registerCollector("time", defaultEnabled, NewTimeCollector)
	fmt.Println("여기는 init 함수 종료부 입니다")
...
func NewTimeCollector(logger log.Logger) (Collector, error) {
	fmt.Println("여기는 NewTimeCollector 향식의 NewTimeCollector 함수 시작부 입니다")
	const subsystem = "time"
	fmt.Println("여기는 NewTimeCollector 향식의 UpdateNewTimeCollector 함수 종료부 입니다")
...
func (c *timeCollector) Update(ch chan<- prometheus.Metric) error {
	fmt.Println("여기는 timeCollector 향식의 Update 함수 시작부 입니다")
	now := time.Now()
	...
	fmt.Println("여기는 timeCollector 향식의 Update 함수 종료부 입니다")
